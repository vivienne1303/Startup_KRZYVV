(function () {
  ...
  const card = (user, follower = false) => `<article class="person-card" data-user-card="${esc(user.id)}"><a href="member-profile.html?id=${encodeURIComponent(user.id)}"><img class="person-avatar" src="${esc(avatar(user))}" alt=""></a><div><h3><a href="member-profile.html?id=${encodeURIComponent(user.id)}">@${esc(user.username || "teenlaunch-user")}</a></h3><p>${esc(user.full_name || t("TeenLaunch user"))}</p><small>${esc(user.school_name || "")}</small></div><div class="person-actions"><button class="btn secondary" type="button" data-follow="${esc(user.id)}" data-following="${user.following || user.followed_back}">${t(user.following || user.followed_back ? "Following" : follower ? "Follow back" : "Follow")}</button><a class="btn primary" href="inbox.html?user=${encodeURIComponent(user.id)}">${t("Message")}</a></div></article>`;
  ...
  const bind = (root) => root.querySelectorAll("[data-follow]").forEach((button) => button.addEventListener("click", async () => {
    ...
    if (response.ok || response.status === 409) { button.dataset.following = String(!following); button.textContent = t(!following ? "Following" : "Follow"); }
  ...
  document.querySelector("[data-search-form]").addEventListener("submit", async (event) => {
    ...
    status.textContent = t("Searching…"); results.innerHTML = "";
    ...
    status.textContent = users.length ? (window.TeenLaunchI18n?.getLanguage() === "zh" ? `找到 ${users.length} 位用户` : `${users.length} user${users.length === 1 ? "" : "s"} found`) : t("No matching usernames found.");
