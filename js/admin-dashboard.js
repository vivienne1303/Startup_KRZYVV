    ...
    document.querySelector("[data-admin-registrations]").innerHTML = registrations.length
      ? registrations
          .map(
            (registration) => `
              <article class="admin-list-item">
                <strong>${escapeHtml(registration.full_name || registration.email || "Applicant")} applied for ${escapeHtml(registration.opportunities?.title || "Opportunity")}</strong>
                <span>${escapeHtml(registration.email || "No email")}${registration.phone_number ? ` · ${escapeHtml(registration.phone_number)}` : ""}${registration.school_name ? ` · ${escapeHtml(registration.school_name)}` : ""}</span>
                <span>Applied through TeenLaunch on ${formatDate(registration.registered_at || registration.created_at)}</span>
              </article>
            `
          )
          .join("")
